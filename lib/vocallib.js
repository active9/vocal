...
var vocallib = function() {
	// Prototype
};

vocallib.prototype.speak = function(voice, txt, cb) {
	var pipedata = '(' + voice +') ( SayText \"' + txt + '\")';
	this.pipe(voice, ['--pipe'], pipedata, cb);
};

vocallib.prototype.sing = function(voice, song, cb) {
	var pipedata = this.tunnel(voice, song, cb);
};

vocallib.prototype.pipe = function(voice, commands, data, cb) {
...
vocallib.prototype.diaphragm = function(pitch) {
    if (pitch.rest) {
        return '<REST BEATS='
...
vocallib.prototype.tunnel = function(voice, song, cb) {
	var self = this;
	var xmlfile = './'
